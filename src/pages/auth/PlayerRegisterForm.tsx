...
export default function PlayerRegisterForm({ onBack }: { onBack?: () => void }) {
    ...
    return (
      <div className="flex min-h-[80vh] items-center justify-center bg-bone px-4 py-16">
        <div className="max-w-md rounded-card border border-grass/30 bg-pitch-soft p-8 text-center">
          ...
          <Button className="mt-6" onClick={() => navigate('/player')}>
  ...
  return (
    <div className="flex min-h-[80vh] items-center justify-center bg-bone px-4 py-16">
      <div className="w-full max-w-sm">
        ...
        {onBack && (
          <button type="button" onClick={onBack} className="mt-4 flex items-center gap-1 text-sm text-pitch/60 hover:text-pitch">
        ...
        <p className="mt-6 text-center text-sm text-pitch/70">
          ...
          <Link to="/login?role=player" className="font-medium text-grass hover:text-grass-bright">
            {t('auth.signIn')}
          </Link>
        </p>
      </div>
